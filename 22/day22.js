...
const testInputs = [
    [
        [
            "on x=0..1,y=1..2,z=2..3",
            "on x=4..5,y=1..2,z=2..3",
            "on x=1..4,y=2..4,z=2..3"
        ], 36
    ],
    [
        [
            "on x=1..5,y=1..2,z=2..3",
            "on x=0..1,y=1..2,z=2..3",
            "on x=3..4,y=1..2,z=2..3",
            "on x=4..7,y=1..2,z=2..3",
        ], 36
    ],
    [
        [
...
let board = {}
setValue = (board, x, y, z, value) => board[`${x}x${y}x${z}`] = value;
getValue = (board, x, y, z) => board[`${x}x${y}x${z}`];

/*
operations.forEach(op => {
    const value = op.instruction === "on";

    if(op.ranges[0][0] < -50 || op.ranges[0][0] > 50) {
        return;
    }

    for(let x = op.ranges[0][0]; x <= op.ranges[0][1]; x++) {
        for(let y = op.ranges[1][0]; y <= op.ranges[1][1]; y++) {
            for(let z = op.ranges[2][0]; z <= op.ranges[2][1]; z++) {
                setValue(board, x, y, z, value);
            }
        }
    }
});*/

const doOverlap = (ra, rb) => {
    let result = ra.xMax >= rb.xMin
        && ra.xMin <= rb.xMax
        && ra.yMax >= rb.yMin
        && ra.yMin <= rb.yMax
        && ra.zMax >= rb.zMin
...
const split = (cube, axis, point) => {
    const result = [{...cube}, {...cube}];

    if(axis === 'x') {
        result[0].xMax = point;
        result[1].xMin = point + 1;
    } else if(axis === 'y') {
        result[0].yMax = point;
        result[1].yMin = point + 1;
    } else if(axis === 'z') {
        result[0].zMax = point;
        result[1].zMin = point + 1;
    }
...
while(operations.length > 0) {
    const cube = operations.shift();
    const overlapping = operations.filter(x => doOverlap(cube, x));
    ...
    const ocube = overlapping[0];

    // Split the cubes into parts
    const xCoords = [cube.xMin, cube.xMax, ocube.xMin, ocube.xMax].sort((a,b) => a - b);
    

    console.log(xCoords);
}*/

parse = (input) => input.map(line => {
    const instructions = line.split(" ");
    const instruction = instructions[0];
    const parts = instructions[1].split(",")
        .map(range => range.substr(2).split("..").map(x => +x));
    return {
        instruction,
        xMin: parts[0][0],
        xMax: parts[0][1],
...
compute = (input) => {
    ...
    const xCoords = [...new Set(operations.flatMap(x => [x.xMin, x.xMax + 1]))].sort((a,b) => a - b);
    const yCoords = [...new Set(operations.flatMap(x => [x.yMin, x.yMax + 1]))].sort((a,b) => a - b);
    const zCoords = [...new Set(operations.flatMap(x => [x.zMin, x.zMax + 1]))].sort((a,b) => a - b);

    //console.log(xCoords, yCoords, zCoords);
    console.log((xCoords.length - 1) * (yCoords.length - 1) * (zCoords.length - 1));
    let count = 0;
    for(let x = 0; x < xCoords.length - 1; x++) {
        for(let y = 0; y < yCoords.length - 1; y++) {
            for(let z = 0; z < yCoords.length - 1; z++) {
                const cube = {
                    xMin: xCoords[x],
                    xMax: xCoords[x + 1] - 1,
                    yMin: yCoords[y],
                ...
                for(let i = operations.length - 1; i >= 0; i--) {
                    if(doOverlap(cube, operations[i])) {
                        if(operations[i].instruction === 'on') {
                            const volume = (cube.xMax - cube.xMin + 1) * (cube.yMax - cube.yMin + 1) * (cube.zMax - cube.zMin + 1);
                            count += volume;
                        }
                        break;
                    }
                ...
                //const overlapping = operations.filter(x => doOverlap(cube, x));
                //console.log(cube, volume, overlapping)
            }
        }
    }
...
compute2 = (input) => {
    ...
    operations.forEach(op => {
        ...
        if(op.xMin < -50 || op.xMin > 50) {
            return;
        }

        for(let x = op.xMin; x <= op.xMax; x++) {
            for(let y = op.yMin; y <= op.yMax; y++) {
                for(let z = op.zMin; z <= op.zMax; z++) {
                    setValue(board, x, y, z, value);
                }
            }
        }
    ...
    return Object.values(board).filter(x => x).length;
}

compute = (input) => {
    let operations = parse(input);
    operations.map((x, i) => x.i = i);
    let done  = [];

    // Seep horizontally to split cubes when needed.
    while(operations.length) {
        //console.log('---HOR--------------------------------- ', operations.length)
        const curr = operations[0];
        const overlapping = operations.filter(x => doOverlap(curr, x));
        overlapping.forEach(x => operations.splice(operations.indexOf(x), 1));
        if(overlapping.length == 0) {
            console.log('ERROR', curr, operations);
            break;
        }

        if(overlapping.length == 1 || !overlapping.some(x => x.xMin !== curr.xMin || x.xMax !== curr.xMax)) {
            //console.log("[DONE] ", overlapping)
            // All the overlapping ones are done (horizontally)
            done.push(...overlapping);
        } else {
            // Get the xCoords
            let newCubes = [];
            const xCoords = [...new Set(overlapping.flatMap(x => [x.xMin, x.xMax + 1]))].sort((a,b) => a - b);
            console.log(xCoords);

            for(let i = 0; i < xCoords.length - 1; i++) {
                // Slice xCoords[i] ... xCoords[i] - 1
                for(let cube of overlapping) {
                    if(cube.xMin <= xCoords[i] && cube.xMax >= xCoords[i]) {
                        // Add the left slice
                        newCubes.push({
                            ...cube,
                            xMax: xCoords[i + 1] - 1
                        })
                        // Let the cube represent the remainder
                        cube.xMin = xCoords[i + 1];
                    }
                }
            }

            console.log(newCubes);
            operations.splice(0, 0, ...newCubes);
            //break;
        }
    }
    // Vertically
    ...
    while(operations.length) {
        //console.log('---VER--------------------------------- ', operations.length)
        const curr = operations[0];
        const overlapping = operations.filter(x => doOverlap(curr, x));
        overlapping.forEach(x => operations.splice(operations.indexOf(x), 1));
        if(overlapping.length == 0) {
        ...
        if(overlapping.length == 1 || !overlapping.some(x => x.yMin !== curr.yMin || x.yMax !== curr.yMax)) {
            //console.log("[DONE] ", overlapping)
            // All the overlapping ones are done (horizontally)
            done.push(...overlapping);
        } else {
            // Get the xCoords
            let newCubes = [];
            const yCoords = [...new Set(overlapping.flatMap(x => [x.yMin, x.yMax + 1]))].sort((a,b) => a - b);
            //console.log(yCoords);

            for(let i = 0; i < yCoords.length - 1; i++) {
                // Slice xCoords[i] ... xCoords[i] - 1
                for(let cube of overlapping) {
                    if(cube.yMin <= yCoords[i] && cube.yMax >= yCoords[i]) {
                        // Add the left slice
                        newCubes.push({
                            ...cube,
                            yMax: yCoords[i + 1] - 1
    ...
    while(operations.length) {
        //console.log('---DEP--------------------------------- ', operations.length)
        const curr = operations[0];
        const overlapping = operations.filter(x => doOverlap(curr, x));
        overlapping.forEach(x => operations.splice(operations.indexOf(x), 1));
        if(overlapping.length == 0) {
        ...
        if(overlapping.length == 1 || !overlapping.some(x => x.zMin !== curr.zMin || x.zMax !== curr.zMax)) {
            //console.log("[DONE] ", overlapping)
            // All the overlapping ones are done (horizontally)
            done.push(...overlapping);
        } else {
            // Get the xCoords
            let newCubes = [];
            const zCoords = [... new Set(overlapping.flatMap(x => [x.zMin, x.zMax + 1]))].sort((a,b) => a - b);
            //console.log(zCoords);

            for(let i = 0; i < zCoords.length - 1; i++) {
                // Slice xCoords[i] ... xCoords[i] - 1
                for(let cube of overlapping) {
                    if(cube.zMin <= zCoords[i] && cube.zMax >= zCoords[i]) {
                        // Add the left slice
                        newCubes.push({
                            ...cube,
                            zMax: zCoords[i + 1] - 1
    ...
    // Remove the duplicates, only leaving the last one.
    const donedone = []
    done.sort((a,b) => a.i - b.i);
    for(let i = done.length - 1; i >= 0; i--) {
        const cube = done[i];
        const overlapping = donedone.filter(x => doOverlap(cube, x));
        if(overlapping.length == 0) {
            donedone.push(cube);
        } else {
            if(overlapping[0].i < cube.i) {
                console.log('ERROR', overlapping[0].i, cube.i);
            } else if(overlapping[0].i === cube.i) {
                console.log('WTF');
            }
        }
    }
    //console.log(done.length, donedone.length);
    const volumes = donedone.map(cube => cube.instruction === 'on' ? (cube.xMax - cube.xMin + 1) * (cube.yMax - cube.yMin + 1) * (cube.zMax - cube.zMin + 1) : 0);
...
compute = (input) => {
    ...
    let state = [];
    for(let cube of operations) {
        let effect = []
        if(cube.instruction === 'on') {
            effect.push(cube);
        }
        for(let ocube of state) {
            if(doOverlap(cube, ocube)) {
                // Create overlapping cube
                overlap = {
                    instruction: cube.instruction,
                    xMin: Math.max(cube.xMin, ocube.xMin),
                    xMax: Math.min(cube.xMax, ocube.xMax),
                    yMin: Math.max(cube.yMin, ocube.yMin),
                    yMax: Math.min(cube.yMax, ocube.yMax),
                    zMin: Math.max(cube.zMin, ocube.zMin),
                    zMax: Math.min(cube.zMax, ocube.zMax)
                }

                // Invert instruction
                overlap.instruction = ocube.instruction === 'on' ? 'off' : 'on';
...
for(let input of testInputs) {
    //input = testInputs[1];
    const result = compute(input[0]);
    if(result != input[1]) {
        console.log("ERROR: Got", result, "but expected", input[1]);
    } else {
