...
let result = input.flatMap(line => line.split(" | ")[1].split(" ")).filter(x => 
    (x.length >=2 && x.length <= 4) || x.length == 7).length
...
let sum = 0;
let parts = input.map(line => line.split(" | ")).map(parts => {
    ...
    //console.log(inputs, hist);
    let lookup = {
        1: inputs.filter(x => x.length == 2)[0],
        2: null,
        3: null,
        4: inputs.filter(x => x.length == 4)[0],
        5: null,
        6: null,
        7: inputs.filter(x => x.length == 3)[0],
        8: inputs.filter(x => x.length == 7)[0],
        9: null,
        0: null,
    };
    //console.log(lookup);

    // Determine true segments
    table = {
        'a': lookup[7].split("").filter(c => lookup[4].split("").indexOf(c) === -1)[0],
        'b': lookup[4].split("").filter(c => hist[c] == 6)[0],
        'c': lookup[1].split("").filter(c => hist[c] == 8)[0],
        'd': lookup[4].split("").filter(c => hist[c] == 7)[0],
        'e': Object.entries(hist).filter(a => a[1] == 4)[0][0],
        'f': lookup[1].split("").filter(c => hist[c] == 9)[0],
        'g': null,
    }
    ...
    translated = inputs.map(x => [...x].map(x => inverse_table[x]).sort().join(''));
    dict = {
        'abcefg': 0,
        'cf': 1,
        'acdeg': 2,
    ...
    translated = +parts[1].split(" ").map(x => [...x].map(x => inverse_table[x]).sort().join(''))
