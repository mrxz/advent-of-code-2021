...
// Part 1
// Digits, 1, 4, 7 and 8 have unique amount of lit segments (2, 4, 3, 7 respectively)
let result = input.flatMap(line => line.split(" | ")[1].split(" "))
    .filter(x => [2, 3, 4, 7].indexOf(x.length) !== -1).length;
...
let sum = 0;
input.map(line => line.split(" | ")).map(parts => {
    let [inputs, outputs] = parts.map(part => part.split(" "));

    // Digits with unique count of lit segments can be determined
    let lookup = {
        1: inputs.filter(x => x.length == 2)[0].split(""),
        4: inputs.filter(x => x.length == 4)[0].split(""),
        7: inputs.filter(x => x.length == 3)[0].split(""),
        8: inputs.filter(x => x.length == 7)[0].split(""),
    };

    // Count the occurences of each segment
    let hist = inputs.flatMap(i => i.split("")).reduce((counters, c) => {
        counters[c] = (counters[c] || 0) + 1
        return counters;
    }, {});
    
    // Determine true segments table
    table = {
        'a': lookup[7].filter(c => lookup[4].indexOf(c) === -1)[0],
        'b': lookup[4].filter(c => hist[c] == 6)[0],
        'c': lookup[1].filter(c => hist[c] == 8)[0],
        'd': lookup[4].filter(c => hist[c] == 7)[0],
        'e': Object.entries(hist).filter(a => a[1] == 4)[0][0],
        'f': lookup[1].filter(c => hist[c] == 9)[0],
        'g': null,
    }
    table['g'] = Object.keys(table).filter(x => Object.values(table).indexOf(x) == -1)[0];

    inverse_table = Object.fromEntries(Object.entries(table).map(x => [x[1], x[0]]));
    ...
    translated = +outputs.map(x => [...x].map(x => inverse_table[x]).sort().join(''))
        .map(key => dict[key]).join('');
    sum += translated;
});
