...
for(let i = 0; i < input.length; i++) {
    const line = input[i];
    if(line === "") {
        // Reached end of scanner, start a new one
        scanner = [];
        scanners.push(scanner);
        continue;
    }

    // Ignore scanner header
    if(line[1] === '-') {
        continue;
    }
    ...
    toCheck.forEach(i => {
        ...
        [...unknown].forEach(j => {
            ...
            if(result) {
                ...
                unknown.splice(unknown.indexOf(j), 1);
                known[j] = result.offset;
                newToCheck.push(j);

                // Update the points of the scanner to the adjusted ones
                // This will ensure that overlaps with it, will already result in
                // new points relative to scanner 0 (as opposed to relative to the scanner itself)
                scanners[j] = result.points;
...
if(unknown.length > 0) {
    console.log("FATAL ERROR: failed to determine positions of all scanners, still unknown:", unknown);
    process.exit()
}

console.log('Part 1:', Object.values(beacons).length);

// Part 2: Compute max Manhattan distance between scanners
let max = 0;
let poses = Object.values(known);
