...
let max = 0;
let poses = Object.values(distance);
                ...
                for(let axis = 0; axis < 3; axis++) {
                    for(let rot = 0; rot < 4; rot++) {
                        ...
                        if(matches >= 12) {
                            ...
                            found = {offset, axis, flip, rot, points: adjusted_pts};
                            break outer;
                        }
...
while(toCheck.length > 0) {
    let newToCheck = [];
    // Always check from the known ones
    toCheck.forEach(i => {
        [...unknown].forEach(j => {
            console.log('-------- ', i, '->', j, '---------');
            ...
            if(result) {
                ...
                // Add the beacons to the set
                const knownOffset = known[i];
                ...
                //scanner[j] = result.new_pts;
                //let absolute_points = result.points.map(p => [p[0] + totalOffset[0], p[1] + totalOffset[1], p[2] + totalOffset[2]]);
                // Override the scanner points with absolute ones
                //scanners[j] = result.points;
                /*
                absolute_points.forEach(p => {
                    const key = `${p[0]}|${p[1]}|${p[2]}`;
                    beacons[key] = (beacons[key] || 0) + 1;
                })*/
            }
...
console.log(known, unknown);
console.log(beacons);
console.log(Object.values(beacons).length);
